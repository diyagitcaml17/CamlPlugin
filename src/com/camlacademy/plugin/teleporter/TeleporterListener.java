...
public class TeleporterListener extends CamlListenerBase {
	...
	public void onCraftItem(PrepareItemCraftEvent event) {
		...
		// there must match the type referred to by
		// CamlPlugin.ITEM_TYPE_TELEPORTER
		if (event.getInventory().getResult().getType() != CamlPlugin.ITEM_TYPE_TELEPORTER) {
			return;
		}
		// there must be items which were used to craft this
		if (event.getInventory().getMatrix() == null || event.getInventory().getMatrix().length < 1) {
			return;
		}
	...
	private boolean playerCanTeleport(String playerName, Date now) {
		...
		boolean playerCanTeleport = (nowMs - lastTeleportTimeMs) / 1000 > TELEPORT_COOL_DOWN_TIME_SEC;
	...
	private void registerRecipes() {
		...
		// teleporter
		plugin.getServer().addRecipe(TeleporterListener.getTeleporterRecipe());
	}
